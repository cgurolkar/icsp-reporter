...
import { initializeDatabase, updateHarcamaKalem } from "@/lib/database"

const PatchSchema = z.object({
  kod: z.string().trim().min(1).max(20).optional(),
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await updateHarcamaKalem(id, parsed.data)
    if (!row) return NextResponse.json({ error: "Bulunamadı." }, { status: 404 })
    return NextResponse.json(row)
  } catch (error) {
    console.error("Harcama kalem PATCH error:", error)
